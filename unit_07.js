...
const f2 = () => {
    ...
    if ( (a2.size === undefined) && (a2.length !== undefined) && (typeof a2 !== 'string') ) {
...
const f12 = () => {
    let res = a12.reduce((accum, item, index, arr) => {
        if(arr[accum] < item) {
...
const f15 = () => {
    ...
    //     sum = (sum + a15[i])% a15.length
    // }
    // console.log(a15.length);

    let res = a15.reduce((accum, item, index, arr) => {
        accum +=item
...
const f17 = () => {
    ...
    console.log(a17_res);
    let res = a17_res.reduce((accum, item) =>{
        accum += ` ${item}` ;
        console.log(accum)
        return accum;
    })
document.querySelector('.out-17').innerHTML = res
}
